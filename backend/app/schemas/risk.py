...
class RiskDecision(BaseModel):
    ...
    # If REJECTED, why?
    rejection_code: Optional[str] = Field(None, description="Standardized rejection code (e.g., 'INSUFFICIENT_FUNDS')")
    rejection_reason: Optional[str] = Field(None, description="Human readable reason")
    
    # If APPROVED or MODIFIED, the final allowed parameters
    calculated_risk: Optional[Decimal] = Field(None, description="Calculated monetary risk if executed")
    calculated_quantity: Optional[Decimal] = Field(None, description="Final approved quantity to trade")
    
    # Which limit was the most restrictive (if modified)
    risk_limit_applied: Optional[str] = Field(None, description="Name of the bounding risk rule")
